...
LastEditTime: 2023-03-21 23:26:44
'''
import numpy as np
import pandas as pd
...
# This neuralnetwork is initially designed as a input-hidden-output model
    ...
    def train_single(self, input_list, target_list):
        ...
        hidden_outputs = self.activation_func(hidden_inputs).reshape((self.hiddennodes, 1))

        # print("hidden_inputs_shape:", hidden_inputs.shape, "\thidden_outputs_shape:", hidden_outputs.shape)

        final_inputs = np.dot(self.w_ho, hidden_outputs)
        final_outputs = self.activation_func(final_inputs).reshape((self.outputnodes, 1))
...
if __name__ == "__main__":
    ...
    # Train the model and show the accuracy in each epochs
    print("Start training:")
    for z in range(CYCLE):
        network.train(data_train_value, data_train_target, 1)
        ...
        print("\tThe model accuracy rate is %.2f %% in %d epochs." % ((accuracy * 100), z+1))
    print("Training is finisheed.", len(list_accuracy))

    plt.plot([i+1 for i in range(CYCLE)], list_accuracy, color='red')
    plt.show()
